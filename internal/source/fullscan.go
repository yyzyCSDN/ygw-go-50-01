...
func (s *FullScanner) Snapshot(ctx context.Context) ([]SnapshotRow, model.Position, error) {
	...
	watermark := model.Position(s.log.MaxPosition())
	return rows, watermark, nil
}

// CurrentWatermark returns the highest source position observed so far.
func (s *FullScanner) CurrentWatermark() model.Position {
	return model.Position(s.log.MaxPosition())
}
