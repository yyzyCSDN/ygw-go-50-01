...
func (m *Mapper) Map(event model.ChangeEvent) (model.ChangeEvent, error) {
	...
	mapped, err := snap.Apply(event)
	if err != nil {
		return event, err
	}
	if rule, ok := m.rules.Filter(mapped.Table); ok {
		keep := make(map[string]bool, len(rule.Keep))
		for _, name := range rule.Keep {
			keep[name] = true
		}
		cols := mapped.Columns[:0]
		for _, col := range mapped.Columns {
			if keep[col.Name] {
